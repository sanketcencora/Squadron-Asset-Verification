...
interface FurnitureDashboardProps {
  userId: string;
}

export function FurnitureDashboard({ userId }: FurnitureDashboardProps) {
  const [showUploadModal, setShowUploadModal] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'furniture' | 'other'>('all');

  // Filter equipment counts for Furniture and Other
  const equipmentData = mockEquipmentCounts.filter(
    eq => eq.category === 'furniture' || eq.category === 'other'
  );
  ...
  const overdueCount = filteredData.filter(eq => eq.verificationStatus === 'Overdue').length;
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Furniture & Fixtures Dashboard</h1>
            <p className="text-gray-600 mt-1">Manage and track furniture, fixtures, and other office equipment</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 mb-8">
        ...
        <div className="p-6">
          <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-lg border-2 border-dashed border-orange-300 p-12 text-center">
            ...
            <div className="bg-white rounded-lg p-4 text-left max-w-2xl mx-auto border border-orange-200">
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">Furniture & Fixtures Inventory</h2>
            <div className="flex items-center space-x-3">
              <select
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value as 'all' | 'furniture' | 'other')}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className={`p-2 rounded-lg mr-3 ${
                        item.category === 'furniture' ? 'bg-orange-50 text-orange-600' : 'bg-gray-50 text-gray-600'
                      }`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            >
              {categories.map(cat => (
                <option key={cat} value={cat}>
                  {cat === 'furniture' ? 'Furniture & Fixtures' : 'Other Equipment'}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Herman Miller Aeron Chairs"
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            />
          </div>
